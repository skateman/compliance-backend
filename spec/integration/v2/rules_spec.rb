  ...
  path '/security_guides/{security_guide_id}/rules' do
    ...
    get 'Request Rules' do
      ...
      response '200', 'Lists Rules' do
        ...
        after { |e| autogenerate_examples(e, 'List of Rules sorted by "title:asc"') }

        run_test!
      end

      response '200', 'Lists Rules' do
        let(:filter) { "(title=#{V2::Rule.first.title})" }
        v2_collection_schema 'rule'

        after { |e| autogenerate_examples(e, "List of Rules filtered by '(#{V2::Rule.first.title})'") }
  ...
  path '/security_guides/{security_guide_id}/rules/{id}' do
    ...
    get 'Request a Rule' do
      ...
      response '404', 'Returns with Not Found' do
        ...
        after { |e| autogenerate_examples(e, 'Description of an error when requesting a non-existing Rule') }
