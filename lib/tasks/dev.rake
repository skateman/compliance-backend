...
namespace :dev do
  ...
  namespace :gabi do
    namespace :seed do
      task :canonicals, %i[token account url] => [:environment] do |_t, args|
        require 'gabi'
      ...
      task :inventory, %i[token account url] => [:environment] do |_t, args|
        require 'gabi'
      ...
      task :userdata, %i[token account url] => [:environment] do |_t, args|
        require 'gabi'
